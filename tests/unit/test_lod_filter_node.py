...
async def test_lod_filter_node_happy_path_high_lod(high_lod_bundle):
    """Test that assets with high LoD distance pass the filter in min mode."""
    node = LodFilter(
        id=1, params={"lod_distance_threshold": 10.0, "atr_window": 14, "filter_mode": "min"}
    )
...
async def test_lod_filter_node_empty_bundle():
    ...
    node = LodFilter(
        id=1, params={"lod_distance_threshold": 3.16, "atr_window": 14, "filter_mode": "min"}
    )
    inputs = {"ohlcv_bundle": {}}
    result = await node.execute(inputs)
...
async def test_lod_filter_node_different_atr_windows():
    ...
    node = LodFilter(
        id=1, params={"lod_distance_threshold": 1.0, "atr_window": 5, "filter_mode": "min"}
    )
    inputs = {"ohlcv_bundle": bundle}
    result = await node.execute(inputs)
...
async def test_lod_filter_node_zero_threshold():
    ...
    node = LodFilter(
        id=1, params={"lod_distance_threshold": 0.0, "atr_window": 14, "filter_mode": "min"}
    )
    inputs = {"ohlcv_bundle": bundle}
    result = await node.execute(inputs)
...
def test_lod_filter_node_parameter_validation():
    ...
    # Valid parameters
    node = LodFilter(
        id=1, params={"lod_distance_threshold": 5.0, "atr_window": 14, "filter_mode": "min"}
    )
    assert node.params["lod_distance_threshold"] == 5.0
    assert node.params["atr_window"] == 14
    assert node.params["filter_mode"] == "min"

    # Invalid lod_distance_threshold
    with pytest.raises(ValueError, match="LoD distance threshold cannot be negative"):
        LodFilter(
            id=1, params={"lod_distance_threshold": -1.0, "atr_window": 14, "filter_mode": "min"}
        )

    # Invalid atr_window
    with pytest.raises(ValueError, match="ATR window must be positive"):
        LodFilter(
            id=1, params={"lod_distance_threshold": 3.16, "atr_window": 0, "filter_mode": "min"}
        )

    # Invalid filter_mode
    with pytest.raises(ValueError, match="Filter mode must be either 'min' or 'max'"):
        LodFilter(
            id=1,
            params={"lod_distance_threshold": 3.16, "atr_window": 14, "filter_mode": "invalid"},
        )
...
async def test_lod_filter_node_progress_reporting():
    ...
    node.set_progress_callback(
        lambda node_id, progress, text: progress_calls.append((progress, text))
    )
...
def test_should_pass_filter_missing_value():
    ...
    missing_result = IndicatorResult(
        indicator_type=IndicatorType.LOD, values=IndicatorValue(lines={"other": 5.0})
    )
...
def test_should_pass_filter_above_threshold():
    ...
    pass_result = IndicatorResult(
        indicator_type=IndicatorType.LOD, values=IndicatorValue(lines={"lod_distance_pct": 10.0})
    )
...
def test_should_pass_filter_below_threshold():
    ...
    node = LodFilter(
        id=1, params={"lod_distance_threshold": 15.0, "atr_window": 14, "filter_mode": "min"}
    )
...
@pytest.mark.asyncio
async def test_lod_filter_node_max_mode_high_lod(high_lod_bundle):
    """Test that assets with high LoD distance fail the filter in max mode."""
    node = LodFilter(
        id=1, params={"lod_distance_threshold": 10.0, "atr_window": 14, "filter_mode": "max"}
    )
    inputs = {"ohlcv_bundle": high_lod_bundle}
    result = await node.execute(inputs)
...
@pytest.mark.asyncio
async def test_lod_filter_node_max_mode_low_lod(low_lod_bundle):
    """Test that assets with low LoD distance pass the filter in max mode."""
    node = LodFilter(
        id=1, params={"lod_distance_threshold": 10.0, "atr_window": 14, "filter_mode": "max"}
    )
    inputs = {"ohlcv_bundle": low_lod_bundle}
...
@pytest.mark.asyncio
async def test_lod_filter_node_max_mode_mixed_bundle(high_lod_bundle, low_lod_bundle):
    """Test filtering with mixed bundle in max mode."""
    mixed_bundle = {**high_lod_bundle, **low_lod_bundle}
    ...
    assert "filtered_ohlcv_bundle" in result
    filtered = result["filtered_ohlcv_bundle"]
    # Should only contain the low LoD symbol
    assert len(filtered) == 1
    high_symbol = AssetSymbol("HIGH_LOD", AssetClass.STOCKS)
    low_symbol = AssetSymbol("LOW_LOD", AssetClass.STOCKS)
    assert high_symbol not in filtered
    assert low_symbol in filtered


def test_should_pass_filter_max_mode_above_threshold():
    """Test _should_pass_filter in max mode with value above threshold."""
    node = LodFilter(
        id=1, params={"lod_distance_threshold": 5.0, "atr_window": 14, "filter_mode": "max"}
    )
    fail_result = IndicatorResult(
        indicator_type=IndicatorType.LOD, values=IndicatorValue(lines={"lod_distance_pct": 10.0})
    )
...
def test_should_pass_filter_max_mode_below_threshold():
    """Test _should_pass_filter in max mode with value below threshold."""
    node = LodFilter(
        id=1, params={"lod_distance_threshold": 15.0, "atr_window": 14, "filter_mode": "max"}
    )
    pass_result = IndicatorResult(
        indicator_type=IndicatorType.LOD, values=IndicatorValue(lines={"lod_distance_pct": 10.0})
    )
