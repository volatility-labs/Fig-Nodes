...
async def test_atrx_filter_node_smoothing(sample_bundle, smoothing):
    ...
    result = await node.execute({"ohlcv_bundle": sample_bundle})
    assert "filtered_ohlcv_bundle" in result
    # With constant data (close=102, high=105, low=95), ATRX calculation depends on smoothing
    # All smoothing methods should produce valid results (ATRX ~0 is inside thresholds)
    assert result["filtered_ohlcv_bundle"] == {}


@pytest.mark.asyncio
async def test_atrx_filter_node_invalid_smoothing(sample_bundle):
    """Test that invalid smoothing method returns error."""
    params = {
        "smoothing": "INVALID",
        "filter_condition": "outside",
        "upper_threshold": 5.0,
    ...
    result = await node.execute({"ohlcv_bundle": sample_bundle})
    assert "filtered_ohlcv_bundle" in result
    # Should return empty bundle due to error
    assert result["filtered_ohlcv_bundle"] == {}
...
def multi_symbol_bundle() -> dict[AssetSymbol, list[OHLCVBar]]:
    bars = [
        {"timestamp": i, "open": 100, "high": 105, "low": 95, "close": 102, "volume": 10000}
        for i in range(60)
    ]
    return {
...
async def test_atrx_filter_node_outside_multi_symbols(multi_symbol_bundle):
    ...
    assert set(filtered.keys()) == {
        AssetSymbol("AAPL", AssetClass.CRYPTO),
        AssetSymbol("TSLA", AssetClass.CRYPTO),
    }
...
async def test_atrx_filter_node_edge_cases_outside(multi_symbol_bundle):
    ...
    assert set(filtered.keys()) == {
        AssetSymbol("AAPL", AssetClass.CRYPTO),
        AssetSymbol("GOOG", AssetClass.CRYPTO),
        AssetSymbol("TSLA", AssetClass.CRYPTO),
    }
...
@pytest.mark.parametrize(
    "condition, upper, lower, expected_kept",
    [
        (
            "outside",
            10.0,
            -10.0,
            ["AAPL", "TSLA"],
        ),  # AAPL (11 >=10), TSLA (-11 <=-10), GOOG (0 inside) filtered out
        ...
        (
            ...
            -0.1,
            ["AAPL", "TSLA"],
        ),  # AAPL (11 >=0.1), TSLA (-11 <=-0.1), GOOG (0 inside, not extreme)
        ...
        (
            ...
            -11.0,
            ["AAPL", "GOOG", "TSLA"],
        ),  # Now includes exact with inclusive (adjusted order)
    ],
)
async def test_atrx_filter_node_varying_params(
    multi_symbol_bundle, condition, upper, lower, expected_kept
):
    ...
    with patch.object(node, "_calculate_indicator") as mock_calc:
        ...
        result = await node.execute({"ohlcv_bundle": multi_symbol_bundle})
    filtered = result["filtered_ohlcv_bundle"]
    assert set(k.ticker for k in filtered.keys()) == set(
        expected_kept
    )  # Use .ticker for string comparison
