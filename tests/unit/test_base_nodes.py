...
from core.types_registry import NodeExecutionError, NodeValidationError, ProgressState
...
def test_base_node_init(base_node):
    ...
    assert base_node.graph_context == {}
...
def test_base_node_init_with_graph_context():
    graph_context = {
    ...
    node = ConcreteBaseNode(id=1, params={}, graph_context=graph_context)
    assert node.graph_context == graph_context
    assert node.graph_context["graph_id"] == "test-graph-123"
    assert node.graph_context["current_node_id"] == 1


def test_base_node_init_without_graph_context():
    node = ConcreteBaseNode(id=1, params={})
    assert node.graph_context == {}
...
def test_report_progress_default_text(base_node):
    ...
    assert call_args["node_id"] == 1
    assert call_args["progress"] == 0.5
    # Empty text is not included in the event dict
