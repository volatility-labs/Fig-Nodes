...
def test_parse_type_list():
    ...
    assert parse_type(typing.List[int]) == {"base": "List", "subtypes": [{"base": "int"}]}
    assert parse_type(typing.List[typing.Any]) == {"base": "List", "subtypes": [{"base": "Any"}]}

def test_parse_type_set():
    assert parse_type(set) == {"base": "set", "subtypes": [{"base": "Any"}]}
    assert parse_type(typing.Set[str]) == {"base": "Set", "subtypes": [{"base": "str"}]}

def test_parse_type_tuple():
    assert parse_type(tuple) == {"base": "tuple", "subtypes": [{"base": "Any"}]}
    assert parse_type(typing.Tuple[int, str]) == {"base": "Tuple", "subtypes": [{"base": "int"}, {"base": "str"}]}

def test_parse_type_dict():
    assert parse_type(dict) == {"base": "dict", "key_type": {"base": "Any"}, "value_type": {"base": "Any"}}
    assert parse_type(typing.Dict[str, int]) == {"base": "Dict", "key_type": {"base": "str"}, "value_type": {"base": "int"}}
    assert parse_type(typing.Dict) == {"base": "Dict", "key_type": None, "value_type": None}  # No args

def test_parse_type_union():
    assert parse_type(typing.Union[int, str]) == {"base": "union", "subtypes": [{"base": "int"}, {"base": "str"}]}
    assert parse_type(typing.Union[int, typing.Any]) == {"base": "union", "subtypes": [{"base": "int"}, {"base": "Any"}]}

def test_parse_type_nested():
    nested = typing.List[typing.Dict[str, typing.Union[int, float]]]
    expected = {
        "base": "List",
        "subtypes": [{
            "base": "Dict",
