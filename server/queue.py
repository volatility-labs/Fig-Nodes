...
import core.graph_executor as graph_executor
from core.node_registry import NodeRegistry
from core.types_registry import SerialisableGraph, ProgressEvent
...
class ExecutionQueue:
    def __init__(self):
        ...
        self._running: Optional[ExecutionJob] = None
        self._id_seq = 0
    ...
    async def get_next(self) -> Optional[ExecutionJob]:
        while True:
            async with self._lock:
                if self._pending:
...
async def _monitor_cancel(job: ExecutionJob, websocket: WebSocket, executor: graph_executor.GraphExecutor, execution_task: Optional[asyncio.Task[Any]] = None):
    ...
    # Normalize task types for type checker
    from typing import Set as _Set, Any as _Any
    tasks: _Set[asyncio.Task[_Any]] = set()
    tasks.add(cancel_wait_task)  # type: ignore
    tasks.add(disconnect_wait_task)  # type: ignore
    if execution_task is not None:
        tasks.add(execution_task)  # type: ignore
...
async def execution_worker(queue: ExecutionQueue, node_registry: NodeRegistry):
    while True:
        ...
        try:
            executor = graph_executor.GraphExecutor(job.graph_data, node_registry)
