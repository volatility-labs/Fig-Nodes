...
class MovingAverageFilter(BaseIndicatorFilter):
    ...
    def _should_pass_filter(self, indicator_result: IndicatorResult) -> bool:
        ...
        if indicator_result.error:
            logger.warning(
                f"❌ MA Filter [{symbol_str}]: Failed due to error: {indicator_result.error}"
            )
            return False
        ...
        if np.isnan(current_price) or np.isnan(current_ma):
            logger.warning(
                f"❌ MA Filter [{symbol_str}]: Failed - NaN values (price={current_price}, MA={current_ma})"
            )
            return False
        ...
        if self.require_price_above_ma:
            if not (current_price > current_ma):
                logger.warning(
                    f"❌ MA Filter [{symbol_str}]: Failed - price {current_price:.4f} <= MA {current_ma:.4f}"
                )
                return False
        ...
        if self.prior_bars == 0:
            if self.require_price_above_ma:
                logger.warning(
                    f"✅ MA Filter [{symbol_str}]: Passed - price {current_price:.4f} > MA {current_ma:.4f} (no slope check)"
                )
            else:
                logger.warning(
                    f"✅ MA Filter [{symbol_str}]: Passed - no price/MA requirement and no slope check (prior_bars=0)"
                )
            return True
        ...
        if slope_positive:
            if self.require_price_above_ma:
                logger.warning(
                    f"✅ MA Filter [{symbol_str}]: Passed - price {current_price:.4f} > MA {current_ma:.4f}, slope POSITIVE ({current_ma:.4f} > {previous:.4f}, diff={slope_diff:.6f})"
                )
            else:
                logger.warning(
                    f"✅ MA Filter [{symbol_str}]: Passed - slope POSITIVE ({current_ma:.4f} > {previous:.4f}, diff={slope_diff:.6f}), price={current_price:.4f}, MA={current_ma:.4f}"
                )
        else:
            if self.require_price_above_ma:
                logger.warning(
                    f"❌ MA Filter [{symbol_str}]: Failed - price {current_price:.4f} > MA {current_ma:.4f}, but slope NEGATIVE ({current_ma:.4f} <= {previous:.4f}, diff={slope_diff:.6f})"
                )
            else:
                logger.warning(
                    f"❌ MA Filter [{symbol_str}]: Failed - slope NEGATIVE ({current_ma:.4f} <= {previous:.4f}, diff={slope_diff:.6f}), price={current_price:.4f}, MA={current_ma:.4f}"
                )
