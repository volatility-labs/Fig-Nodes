...
import { calculateVbp } from '../../../../services/indicator-calculators';
...
const PRICE_ROUNDING_PRECISION = 2;
...
export class VBPLevelFilter extends BaseIndicatorFilter {
  ...
  private getPriceLevel(level: Record<string, unknown>): number {
    const pl = level.priceLevel;
    return typeof pl === 'number' ? pl : 0;
  }

  private buildIndicatorResult(
    ohlcvData: OHLCVBar[],
    allLevels: Array<Record<string, unknown>>
  ): IndicatorResult {
    ...
    const uniqueLevels: Array<Record<string, unknown>> = [];
    ...
    if (useWeekly) {
      ...
      const processSymbol = async (symbolKey: string, ohlcvData: OHLCVBar[]): Promise<void> => {
        try {
          ...
          console.error(`Error processing VBP for ${symbolKey}:`, error);
        }
      };
