...
import { IndicatorType, createIndicatorResult, createIndicatorValue, AssetSymbol } from '../../../../core/types';
import type {
  ParamMeta,
  DefaultParams,
  OHLCVBar,
...
import { calculateSma, calculateEma } from '../../../../services/indicator-calculators';
...
export class MovingAverageFilter extends BaseIndicatorFilter {
  ...
  protected override validateIndicatorParams(): void {
    let periodValue = this.params.period ?? 200;
    let priorBarsValue = this.params.prior_bars ?? 1;
    const maTypeValue = this.params.ma_type ?? 'SMA';
    let requirePriceAboveMaValue = this.params.require_price_above_ma ?? 'true';

    // Convert period to integer
    if (typeof periodValue === 'string') {
      periodValue = parseInt(periodValue, 10);
    ...
    if (typeof priorBarsValue === 'string') {
      priorBarsValue = parseInt(priorBarsValue, 10);
    } else if (typeof priorBarsValue === 'number') {
      priorBarsValue = Math.floor(priorBarsValue);
    } else {
      throw new Error('prior_bars must be an integer');
    }

    if (maTypeValue !== 'SMA' && maTypeValue !== 'EMA') {
      throw new Error("ma_type must be 'SMA' or 'EMA'");
    }

    // Convert string "true"/"false" to boolean
    if (typeof requirePriceAboveMaValue === 'string') {
      requirePriceAboveMaValue = requirePriceAboveMaValue.toLowerCase() === 'true';
    } else if (typeof requirePriceAboveMaValue !== 'boolean') {
      throw new Error("require_price_above_ma must be 'true' or 'false'");
    }

    this.period = periodValue;
    this.priorBars = priorBarsValue;
    this.maType = maTypeValue as 'SMA' | 'EMA';
  ...
  protected calculateIndicator(ohlcvData: OHLCVBar[]): IndicatorResult {
    ...
    if (ohlcvData.length < this.period) {
      ...
      return createIndicatorResult(
        ...
        createIndicatorValue(0, {}),
        {},
        `Insufficient data: ${ohlcvData.length} bars < ${this.period}`
      );
    }
    ...
    if (Number.isNaN(currentMa)) {
      return createIndicatorResult(
        ...
        createIndicatorValue(0, {}),
        {},
        `Unable to compute current ${this.maType}`
      );
    }
    ...
    if (this.priorBars === 0) {
      ...
      return createIndicatorResult(
        indicatorType,
        lastTs,
        createIndicatorValue(0, { current: currentMa, previous: NaN, price: currentPrice }),
        { period: this.period, ma_type: this.maType }
      );
    }
    ...
    if (lookbackIndex < this.period) {
      ...
      return createIndicatorResult(
        indicatorType,
        lastTs,
        createIndicatorValue(0, { current: NaN, previous: NaN, price: currentPrice }),
        {},
        `Insufficient data for ${this.priorBars} bar lookback with ${this.period} period MA`
      );
    }
    ...
    if (previousData.length < this.period) {
      ...
      return createIndicatorResult(
        ...
        createIndicatorValue(0, { current: NaN, previous: NaN, price: currentPrice }),
        {},
        `Insufficient data for previous ${this.maType}`
      );
    }
    ...
    if (Number.isNaN(previousMa)) {
      ...
      return createIndicatorResult(
        ...
        createIndicatorValue(0, { current: currentMa, previous: NaN, price: currentPrice }),
        {},
        `Unable to compute previous ${this.maType}`
      );
    }
    ...
    return createIndicatorResult(
      indicatorType,
      lastTs,
      createIndicatorValue(0, { current: currentMa, previous: previousMa, price: currentPrice }),
      { period: this.period, ma_type: this.maType }
    );
  }
