...
export class WildersADXFilter extends BaseIndicatorFilter {
  ...
  protected calculateIndicator(ohlcvData: OHLCVBar[]): IndicatorResult {
    ...
    const lookbackParam = this.params.di_lookback_bars;
    const lb = Math.max(1, Math.floor(typeof lookbackParam === 'number' ? lookbackParam : 3));
    ...
    return createIndicatorResult({
      ...
      values: createIndicatorValue({
        single: latestAdx ?? 0.0,
        lines: { pdi: latestPdi ?? 0.0, ndi: latestNdi ?? 0.0 },
        series: tail,
      }),
      params: this.params,
    });
