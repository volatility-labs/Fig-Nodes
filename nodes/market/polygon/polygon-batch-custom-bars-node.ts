...
// Translated from: nodes/custom/polygon/polygon_batch_custom_bars_node.py

import { Base, RateLimiter } from '@fig-node/core';
import {
  AssetSymbol,
  NodeCategory,
  NodeUIConfig,
 ...
 */
export class PolygonBatchCustomBars extends Base {
  static required_keys = ['POLYGON_API_KEY'];
  static inputs = { symbols: getType('AssetSymbolList') };
  static outputs = { ohlcv_bundle: getType('OHLCVBundle') };
  ...
  protected async executeImpl(
    ...
    for (let i = 0; i < symbols.length; i += effectiveConcurrency) {
      ...
      this.reportProgress(progress, `Fetched ${completed}/${total} symbols`);
    }
