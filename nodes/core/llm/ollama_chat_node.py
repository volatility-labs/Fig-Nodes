...
class OllamaChat(Base):
    ...
    async def _resolve_max_context(self, host: str, model: str) -> int | None:
        """
    ...
    async def _apply_context_window(
        self, host: str, model: str, options: dict[str, Any]
    ) -> dict[str, Any]:
        """
    ...
    @staticmethod
    def _build_messages(
        existing_messages: list[dict[str, Any]] | None, prompt: str | None, system_input: Any | None
    ) -> list[dict[str, Any]]:
        """
    ...
    def _parse_options(self) -> dict[str, Any] | None:
    ...
    def _get_effective_host(self, inputs: dict[str, Any] | None) -> str:
    ...
    def _get_keep_alive_value(self) -> Any | None:
        """Preserve explicit 0 and duration strings; only None/empty -> None."""
        _keep_alive_param = self.params.get("keep_alive")
        return (
            _keep_alive_param
            if (_keep_alive_param is not None and _keep_alive_param != "")
            else None
        )

    def _get_format_value(self) -> str | None:
    ...
    async def _get_model(self, host: str, model_from_input: str | None) -> str:
        if model_from_input:
        ...
        if not selected and not models_list:
            ...
            logger.warning(
                f"OllamaChatNode: WARNING - Selected model '{selected}' not in available models {models_list}"
            )
                    ...
                    messages.append(
                        {
                            "role": "tool",
                            "tool_name": str(
                                (call or {}).get("function", {}).get("name", "unknown")
                            ),
                            "content": json.dumps(
                                {"error": "handler_failure", "message": str(_e_outer)}
                            ),
                        }
                    )
    ...
    async def _execute_impl(self, inputs: dict[str, Any]) -> dict[str, Any]:
        ...
        raw_messages: list[dict[str, Any]] | None = inputs.get("messages")
        prompt_text: str | None = inputs.get("prompt")
        system_input: Any | None = inputs.get("system")
        ...
        fmt: str | None = self._get_format_value()
        ...
        try:
            ...
            if tools:
                ...
                cancel_wait = asyncio.create_task(self._cancel_event.wait())
                    ...
                    {chat_task, cancel_wait}, return_when=asyncio.FIRST_COMPLETED
                )
