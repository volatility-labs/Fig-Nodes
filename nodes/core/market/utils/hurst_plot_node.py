...
from services.indicator_calculators.cycle_channel_oscillator_calculator import (
    calculate_cycle_channel_oscillator,
)
...
def _plot_candles(ax: "Axes", series: list[tuple[int, float, float, float, float]], volumes: list[float] | None = None, ema_10: list[float | None] | None = None, ema_30: list[float | None] | None = None, ema_100: list[float | None] | None = None, show_current_price: bool = True, current_price_override: float | None = None) -> None:
    ...
    # Stagger annotations vertically to avoid overlap when EMAs are close together
    if ema_lines and len(series) > 0:
        last_index = len(series) - 1
        # Collect all valid EMA values first to determine vertical spacing
        valid_emas = []
        for ema_values, color, label, _linewidth in ema_data:
            if ema_values and len(ema_values) == len(series) and ema_values[last_index] is not None:
                ...
                if isinstance(ema_value, (int, float)):
                    valid_emas.append((ema_value, color, label, _linewidth))
        
        # Sort by EMA value (lowest to highest) for consistent ordering
        valid_emas.sort(key=lambda x: x[0])
        
        # Annotate each EMA with staggered vertical offset to prevent overlap
        # Stack from highest EMA value (top) to lowest EMA value (bottom)
        for idx, (ema_value, color, label, _linewidth) in enumerate(valid_emas):
            # Stagger vertically: highest EMA at top, lowest at bottom
            # Use 20 points spacing between annotations for clear separation
            # idx 0 (lowest) gets most negative offset, highest gets most positive
            vertical_offset = 20 * (idx - (len(valid_emas) - 1) / 2)
            
            # Position annotation to the right of the last bar with vertical stagger
            ax.annotate(
                ...
                color=color,
                fontweight='bold',
                bbox=dict(boxstyle="round,pad=0.4", facecolor='white', edgecolor=color, alpha=0.95, linewidth=2.0),
                ha='left',
                va='center'
            )
        
        # Add a summary box with all EMA values together for AI clarity (prevents misreading)
        if len(valid_emas) >= 2:  # Only show if we have at least 2 EMAs
            # Create summary text with all EMA values
            ema_summary_lines = []
            for ema_value, color, label, _linewidth in sorted(valid_emas, key=lambda x: x[0], reverse=True):  # Highest to lowest
                ema_summary_lines.append(f"{label}: ${ema_value:.2f}")
            
            summary_text = "\n".join(ema_summary_lines)
            
            # Position summary box at top-right, above the EMA annotations
            # Get the highest EMA value for vertical positioning
            highest_ema = max(valid_emas, key=lambda x: x[0])[0]
            
            # Add summary box annotation
            ax.annotate(
                summary_text,
                xy=(last_index, highest_ema),
                xytext=(5, 50),  # Position well above EMA annotations
                textcoords="offset points",
                fontsize=9,
                fontweight='bold',
                bbox=dict(boxstyle="round,pad=0.5", facecolor='#f0f0f0', edgecolor='#333333', alpha=0.95, linewidth=2.0),
                ha='left',
                va='bottom',
...
def _plot_cycle_channel_oscillator(
    ax: "Axes",
    timestamps: list[int],
    fast_osc: list[float | None] | None = None,
    slow_osc: list[float | None] | None = None,
) -> None:
    """Plot Cycle Channel Oscillator on the given axes."""
    if not timestamps or (fast_osc is None and slow_osc is None):
        return
    ...
    # Plot fast oscillator (red) and slow oscillator (green)
    if fast_osc and len(fast_osc) == len(timestamps):
        ax.plot(x_indices, fast_osc, color="#f44336", linewidth=2.0, alpha=0.9, label="Fast Osc", linestyle='-')
    
    if slow_osc and len(slow_osc) == len(timestamps):
        ax.plot(x_indices, slow_osc, color="#4caf50", linewidth=2.0, alpha=0.9, label="Slow Osc", linestyle='-')
    
    # Plot reference lines at 0.0, 0.5, and 1.0
    ...
    # Highlight extreme conditions (above 1.0 or below 0.0) with purple histogram
    if fast_osc and len(fast_osc) == len(timestamps):
        for i, val in enumerate(fast_osc):
            if val is not None:
                if val >= 1.0:
                    ax.bar(i, val - 1.0, bottom=1.0, color="#9c27b0", alpha=0.6, width=0.8, linewidth=0)
                elif val <= 0.0:
                    ax.bar(i, abs(val), bottom=0.0, color="#9c27b0", alpha=0.6, width=0.8, linewidth=0)
    
    if slow_osc and len(slow_osc) == len(timestamps):
        for i, val in enumerate(slow_osc):
            if val is not None:
                if val >= 1.0:
                    ax.bar(i, val - 1.0, bottom=1.0, color="#9c27b0", alpha=0.6, width=0.8, linewidth=0)
                elif val <= 0.0:
...
class HurstPlot(Base):
    ...
    default_params = {
        ...
        # Cycle Channel Oscillator parameters
        "show_cco": False,  # On/off switch for Cycle Channel Oscillator
        "cco_short_cycle_length": 10,  # Short cycle length (default: 10)
        "cco_medium_cycle_length": 30,  # Medium cycle length (default: 30)
        "cco_short_cycle_multiplier": 1.0,  # Short cycle multiplier (default: 1.0)
        "cco_medium_cycle_multiplier": 3.0,  # Medium cycle multiplier (default: 3.0)
    }
    ...
    params_meta = [
        ...
        },
        {
            ...
            "default": False,
            "options": [True, False],
            "description": "Show Cycle Channel Oscillator",
        },
        {
        ...
        },
        {
            ...
            "max": 10.0,
            "step": 0.1,
            "description": "CCO Short Cycle Multiplier",
        },
        {
            ...
            "max": 10.0,
            "step": 0.1,
            "description": "CCO Medium Cycle Multiplier",
        },
    ]
    ...
    async def _execute_impl(self, inputs: dict[str, Any]) -> dict[str, Any]:
        ...
        cco_data_by_symbol: dict[str, dict[str, Any]] = {}  # Store Cycle Channel Oscillator data per symbol
        ...
        for sym, bars in items:
            ...
            # Calculate and plot Cycle Channel Oscillator if enabled
            if show_cco:
                try:
                    # Get CCO parameters
                    ...
                    # Extract highs, lows, closes for CCO calculation
                    full_highs = [bar[2] for bar in full_norm]
                    full_lows = [bar[3] for bar in full_norm]
                    full_closes = [bar[4] for bar in full_norm]
                    
                    # Calculate CCO with full history
                    cco_result = calculate_cycle_channel_oscillator(
                        closes=full_closes,
                        highs=full_highs,
                        lows=full_lows,
                    ...
                    display_fast_osc = cco_result["fast_osc"][start_idx:] if len(cco_result["fast_osc"]) > start_idx else []
                    display_slow_osc = cco_result["slow_osc"][start_idx:] if len(cco_result["slow_osc"]) > start_idx else []
                    
                    # Plot CCO on fourth subplot (or third if MESA not enabled)
                    cco_ax = ax4 if show_mesa else ax3
                    if cco_ax is not None:
                        _plot_cycle_channel_oscillator(cco_ax, timestamps, display_fast_osc, display_slow_osc)
                        cco_ax.set_title(f"{str(sym)} Cycle Channel Oscillator", fontsize=12, fontweight='bold', pad=10)
                        cco_ax.set_ylabel("CCO", fontsize=10)
                        cco_ax.set_xlabel("Bar Index", fontsize=10)
                        cco_ax.tick_params(labelsize=9)
                    
                except Exception as e:
                    logger.error(f"Error calculating Cycle Channel Oscillator for {sym}: {e}", exc_info=True)
        ...
        return {
            ...
            "cco_data": cco_data_by_symbol,  # Cycle Channel Oscillator data (when enabled, empty dict otherwise)
        }
