...
from core.types_registry import get_type, OHLCVBar, AssetSymbol
...
class PriceDataFetching(Base):
    ...
    async def _execute_impl(self, inputs: dict[str, Any]) -> dict[str, Any]:
        ...
        for symbol, ohlcv_data in ohlcv_bundle.items():
            if not ohlcv_data or not isinstance(ohlcv_data, list) or len(ohlcv_data) == 0:
            ...
            if not isinstance(price, (int, float)):
                logger.warning(f"PriceDataFetching node received invalid price value for {symbol}: {price}")
                continue
            ...
            output_lines.append(f"{str(symbol):15} ${price:>10.4f}  @  {timestamp_str}  ({age_str} old)")
        ...
        if save_to_csv and csv_rows:
            try:
                ...
                results_dir = Path(__file__).resolve().parent.parent.parent.parent.parent / "results"
                ...
                with open(csv_path, 'w', newline='') as csvfile:
                    fieldnames = ['symbol', 'price', 'bar_timestamp', 'age_minutes', 'open', 'high', 'low', 'volume']
