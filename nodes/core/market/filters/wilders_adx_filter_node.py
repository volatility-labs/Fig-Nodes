...
class WildersADXFilter(BaseIndicatorFilter):
    ...
    params_meta = [
        ...
        {"name": "direction", "type": "combo", "default": "any", "options": ["any", "bullish", "bearish"]},
    ...
    def _should_pass_filter(self, indicator_result: IndicatorResult) -> bool:
        ...
        if not isinstance(latest_adx, (int, float)):
            return False
        if latest_adx < float(min_adx):
            return False

        pdi = float(lines.get("pdi", 0.0)) if isinstance(lines, dict) else 0.0
        ndi = float(lines.get("ndi", 0.0)) if isinstance(lines, dict) else 0.0
        ...
        if require_adx_rising and isinstance(series, list) and len(series) >= 2:
        ...
        if require_crossover and isinstance(series, list) and len(series) >= 2:
            crossed = False
