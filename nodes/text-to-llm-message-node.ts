...
export class TextToLLMMessage extends Node {
  ...
  private formatDictReadable(data: Record<string, unknown>): string {
    // Check for special types first, using early return pattern
    // Store reference to avoid type narrowing issues with 'in' operator
    const originalData = data;

    if (this.isOhlcvBar(originalData)) {
      return this.formatOhlcvBar(originalData);
    }
    if (this.isChatMessage(originalData)) {
      return this.formatChatMessage(originalData);
    }
    if (this.isIndicatorResult(originalData)) {
      return this.formatIndicatorResult(originalData);
    }

    // Special case: dict containing ohlcv list (Polygon API format)
    const ohlcvValue = originalData['ohlcv'];
    if (Array.isArray(ohlcvValue)) {
      return this.formatOhlcvData(ohlcvValue as Record<string, unknown>[]);
    }

    // Generic dict formatting
    return this.formatGenericDict(originalData);
  }
