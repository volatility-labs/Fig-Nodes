...
def calculate_bandpass_filter(
    ...
    # IMPORTANT: Pinescript uses 3.14 as approximation of pi, not np.pi!
    # To match TradingView exactly, we must use the same approximation
    PI_APPROX = 3.14  # Pinescript approximation (not np.pi!)
    beta = np.cos(PI_APPROX * (360.0 / period) / 180.0)
    gamma = 1.0 / np.cos(PI_APPROX * (720.0 * bandwidth / period) / 180.0)
    ...
    for i in range(len(series_list)):
        ...
        current_val = current
        two_bars_ago_val = series_list[i - 2] if series_list[i - 2] is not None else current_val
        ...
        result[i] = float(bpf)  # Convert numpy float64 to Python float
...
def calculate_hurst_oscillator(
        ...
        for i in range(len(source_series)):
            ...
            for period_name, bpf_values in bandpasses.items():
                if composite_selection.get(period_name, False):
                    if i < len(bpf_values) and bpf_values[i] is not None:
                        comp_value += bpf_values[i]
                        has_value = True
    ...
    if len(peaks) > 1:
        ...
        wavelength = float(np.mean(peak_distances)) if peak_distances else None
    elif len(troughs) > 1:
        trough_distances = [troughs[i] - troughs[i - 1] for i in range(1, len(troughs))]
        wavelength = float(np.mean(trough_distances)) if trough_distances else None
    ...
    if composite:
        ...
        if valid_values:
            ...
            amplitude = float(max_val - min_val)  # Convert to Python float
