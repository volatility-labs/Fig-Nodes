...
import type { IBaseWidget, IDOMWidget } from "./types/widgets"
...
export class LGraphCanvas implements CustomEventDispatcher<LGraphCanvasEventMap> {
  ...
  drawFrontCanvas(): void {
    ...
    // Update DOM widget positions after all canvas drawing is complete
    this.updateDOMWidgetPositions()

    if (area) ctx.restore()
  }

  /**
   * Update positions of all DOM widgets to match their nodes on the canvas.
   * Called after each canvas draw to keep DOM elements synchronized.
   */
  updateDOMWidgetPositions(): void {
    if (!this.graph) return

    const { scale, offset } = this.ds
    const zoomThreshold = LiteGraph.DOM_WIDGET_ZOOM_THRESHOLD ?? 0.5
    ...
    for (const node of this.graph._nodes) {
      ...
      for (const widget of node.widgets) {
        ...
        const domWidget = widget as IDOMWidget
        if (!domWidget.element) continue

        const isVisible = this.isNodeVisible(node) && !node.flags?.collapsed
        const shouldHideForZoom = isLowQuality && (domWidget.domOptions?.hideOnZoom !== false)

        if (!isVisible || shouldHideForZoom) {
          domWidget.element.style.display = "none"
          continue
        }

        this.positionDOMWidget(node, domWidget, scale, offset)
      }
    }
  }

  /**
   * Position a single DOM widget element to match its node position on canvas.
   * @param node The node containing the widget
   * @param widget The DOM widget to position
   * @param scale Current canvas scale
   * @param offset Current canvas offset
   */
  positionDOMWidget(node: LGraphNode, widget: IDOMWidget, scale: number, offset: Point): void {
    ...
    // Check if widget is within visible bounds
    ...
    el.style.width = `${Math.max(0, screenW)}px`
    el.style.height = `${Math.max(0, screenH)}px`
