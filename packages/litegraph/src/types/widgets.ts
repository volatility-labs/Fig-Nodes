...
/**
 * Options for DOM-based widgets (textarea, code editor, etc.)
 * These widgets render as HTML elements overlaid on the canvas.
 */
export interface IDOMWidgetOptions {
  /** Hide widget when zoomed out below threshold (default: true) */
  hideOnZoom?: boolean
  /** Zoom level below which to hide widget (default: 0.5) */
  zoomThreshold?: number
  /** Get the current widget value */
  getValue?: () => unknown
  /** Set the widget value */
  setValue?: (value: unknown) => void
  /** Get minimum height for layout */
  getMinHeight?: () => number
  /** Get maximum height for layout */
  getMaxHeight?: () => number
}

/**
 * DOM widget interface - represents widgets backed by HTML elements.
 * These are positioned over the canvas and follow the node as it moves.
 */
export interface IDOMWidget extends IBaseWidget<string | object, "dom"> {
  type: "dom"
  /** The DOM widget subtype (textarea, code, etc.) */
  domWidgetType?: string
  /** The HTML element for this widget */
  element: HTMLElement
  /** DOM-specific options */
  domOptions?: IDOMWidgetOptions
}
