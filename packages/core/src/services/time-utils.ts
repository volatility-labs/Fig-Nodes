// src/services/time-utils.ts
// Translated from: services/time_utils.py

import { AssetClass } from '../core/types';
...
export function etTimeToUtcTimestampMs(
  ...
  // Create a date string that represents the ET time
  // Note: JavaScript months are 0-indexed
  const dateStr = `${year}-${String(month).padStart(2, '0')}-${String(day).padStart(2, '0')}T${String(hour).padStart(2, '0')}:${String(minute).padStart(2, '0')}:00`;

  // Create date and get UTC timestamp
  // We need to account for ET offset
...
export function utcTimestampMsToEtDatetime(timestampMs: number): Date {
  ...
  // Return the date - the consumer should format it in ET timezone
  return date;
}
...
export function utcTimestampFlexToEtDatetime(ts: number): Date | null {
  ...
  // Try all candidates in order: ns -> ms -> s
  const candidates = [
...
export function utcTimestampMsToDatetime(
  ...
  const date = new Date(timestampMs);
  // The Date object is always UTC internally
  // Formatting in specific timezone is done when displaying
  return date;
}
