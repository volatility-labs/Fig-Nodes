 ...
 */
function isConcreteNodeClass(obj) {
    ...
    try {
        ...
        while (proto) {
            ...
            if (proto instanceof Base || proto.constructor.name === 'Base') {
                ...
                const hasExecuteImpl = objProto['executeImpl'] !== baseProto['executeImpl'] ||
 ...
 */
export async function loadNodes(directories) {
    const registry = {};
                ...
                for (const [_exportName, exportValue] of Object.entries(module)) {
                    if (isConcreteNodeClass(exportValue)) {
                        ...
                        if (className && className !== 'Base') {
                            ...
                            registry[className] = exportValue;
                            // Validate that the node defines its own uiConfig
                            const nodeClass = exportValue;
                            if (nodeClass._isBaseUiConfig === true) {
                                console.warn(`⚠️  Node "${className}" does not define its own uiConfig. ` +
 ...
 */
export async function getNodeRegistry(dirs) {
    if (_nodeRegistry) {
