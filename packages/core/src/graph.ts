// src/graph.ts
// Graph schema, factory, edge helpers, pure mutation functions, and validation

import type { NodeRegistry, PortDef } from './types.js';
...
function validateEdgeTypes(
  ...
  for (let i = 0; i < doc.edges.length; i++) {
    ...
    const SourceClass = nodeRegistry[sourceNode.type] as
      | (Function & { definition?: { outputs?: PortDef[] } })
      | undefined;
    const TargetClass = nodeRegistry[targetNode.type] as
      | (Function & { definition?: { inputs?: PortDef[] } })
      | undefined;
    if (!SourceClass || !TargetClass) continue;

    const outputSpec = (SourceClass.definition?.outputs ?? []).find((p) => p.name === from.portName);
    const inputSpec = (TargetClass.definition?.inputs ?? []).find((p) => p.name === to.portName);
    if (outputSpec == null) {
      errors.push({
        path: `edges[${i}]`,
    ...
    const outputSocketKey = getSocketKey(outputSpec);
    const inputSocketKey = getSocketKey(inputSpec);
