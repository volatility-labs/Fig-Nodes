...
function patchLiteGraphWheelEvents(): void {
    // Only patch once
...
export class EditorInitializer {
    ...
    async createEditor(container: HTMLElement): Promise<EditorInstance> {
        try {
            // Apply wheel event patch before creating canvas instances
            patchLiteGraphWheelEvents();
            ...
            // Initialize ChartManager for rendering chart modals
            const chartManager = new ChartManager(this.serviceRegistry);
            this.serviceRegistry.register('chartManager', chartManager);
