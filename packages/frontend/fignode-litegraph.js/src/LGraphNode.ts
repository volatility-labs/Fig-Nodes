...
import type { IBaseWidget, IDOMWidget, IDOMWidgetOptions, IWidgetOptions, TWidgetType, TWidgetValue } from "./types/widgets"
...
export class LGraphNode implements NodeLike, Positionable, IPinnable, IColorable {
  ...
  /**
   * Add a DOM-based widget to the node.
   * DOM widgets are HTML elements that overlay the canvas and follow the node.
   * @param name Unique name for the widget
   * @param widgetType The type of DOM widget (e.g., 'textarea', 'code')
   * @param element The HTML element to use for this widget
   * @param options DOM widget options (hideOnZoom, getValue, setValue, etc.)
  ...
  addDOMWidget(
    ...
    widgetType: string,
    element: HTMLElement,
    options?: IDOMWidgetOptions,
  ): IDOMWidget {
    const initialValue = options?.getValue?.()
    let widgetValue: string | object | undefined
    if (typeof initialValue === "string") {
      ...
      widgetValue = String(initialValue)
    }

    const widget: IDOMWidget = {
      type: "dom",
      domWidgetType: widgetType,
      name,
      element,
      domOptions: options,
      value: widgetValue,
