...
describe('Strict connection typing', () => {
    ...
    test('parseType handles union types correctly', () => {
        ...
        // Union types use comma separator for LiteGraph compatibility
    ...
    test('parseType handles simple types correctly', () => {
        ...
        expect(typeColorRegistry.parseType({ base: 'Any' })).toBe(0); // Wildcard
    });
    ...
    test('parseType passes through plain strings', () => {
        // Plain strings are passed through as-is (no pipe conversion needed since backend sends structured types)
    ...
    test('node input slots have comma-separated union types', () => {
        // Create node with union input - verify the parsed type is comma-separated
        ...
        // The input type should now be comma-separated for LiteGraph compatibility
